...
exports.lexicalSimilarity = lexicalSimilarity;
const STOPWORDS = new Set(("a an and are as at be by for from has have he her his i in is it its " +
    ...
    "including include includes etc plus new join us looking seeking ideal day days week per month").split(/\s+/));
...
function lexicalSimilarity(a, b) {
    ...
    for (const t of tokenize(a))
        va.set(t, (va.get(t) ?? 0) + 1);
    for (const t of tokenize(b))
        vb.set(t, (vb.get(t) ?? 0) + 1);
    let dot = 0;
    for (const [t, c] of va)
        dot += c * (vb.get(t) ?? 0);
