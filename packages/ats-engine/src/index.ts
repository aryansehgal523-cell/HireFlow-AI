...
const STRONG_VERBS = new Set(
  (
    "led built shipped launched designed architected drove increased reduced grew scaled " +
    ...
    "generated saved cut transformed pioneered orchestrated achieved won secured raised"
  ).split(/\s+/)
);
...
export function resumeToText(r: ResumeContent): string {
  const parts: string[] = [];
  if (r.basics.headline) parts.push(r.basics.headline);
...
function scoreKeywords(resumeText: string, jd: string) {
  const jdTerms = extractTerms(jd, 30);
...
function scoreSkillsMatch(resume: ResumeContent, jd: string): { sub: SubScore; missingSkills: string[] } {
  const jdSkills = [...new Set(extractSkills(jd))];
  const resumeSkillText = [resumeToText(resume)].join(" ");
  const resumeSkills = new Set([
    ...resume.skills.map((s) => normalize(s)),
    ...extractSkills(resumeSkillText),
  ]);
  ...
  const matched = jdSkills.filter((s) => resumeSkills.has(s) || normalize(resumeSkillText).includes(s));
...
function scoreExperienceRelevance(resume: ResumeContent, jd: string): SubScore {
  const expText = resume.experience
    .map((e) => `${e.title} ${e.company} ${e.bullets.join(" ")}`)
    .join("\n");
  const sim = lexicalSimilarity(expText, jd); // 0..1, typical good match ~0.25-0.5
...
function scoreEducationRelevance(resume: ResumeContent, jd: string): SubScore {
  ...
  const sim = lexicalSimilarity(eduText, jd);
  return {
...
function scoreImpactQuality(resume: ResumeContent): SubScore {
  const bullets = [
  ...
  for (const b of bullets) {
    ...
    if (STRONG_VERBS.has(first)) strongStart++;
    if (WEAK_PHRASES.some((w) => b.toLowerCase().includes(w))) weak++;
  }
  ...
  if (sRatio < 0.6) details.push("Start more bullets with strong action verbs (Led, Built, Reduced…).");
  if (weak > 0) details.push(`Remove weak phrasing like "responsible for" (${weak} found).`);
  return { score, weight: WEIGHTS.impactQuality, details };
}

function scoreReadability(resume: ResumeContent): SubScore {
  ...
  const totalWords = tokenize(resumeToText(resume)).length;
  if (totalWords > 900) {
    score -= 15;
  ...
  if (details.length === 0) details.push("Length and bullet structure are recruiter-friendly.");
  return { score: clamp(score), weight: WEIGHTS.readability, details };
}

function scoreAtsCompatibility(resume: ResumeContent): SubScore {
  const details: string[] = [];
  let score = 100;
  ...
  if (details.length === 0) details.push("Structure parses cleanly: contact info, dated experience, and a skills section.");
  return { score: clamp(score), weight: WEIGHTS.atsCompatibility, details };
}

export function scoreResume(resume: ResumeContent, jobDescription?: string): AtsResult {
  ...
  const overall = clamp(
    Object.values(breakdown).reduce((s, b) => s + b.score * b.weight, 0)
  );

  const suggestions: string[] = [];
  for (const sub of Object.values(breakdown)) {
    if (sub.score < 75) suggestions.push(...sub.details);
  }
  if (kw && kw.missing.length > 0) {
    suggestions.push(
      `Work these job-description terms into your bullets where true: ${kw.missing.slice(0, 8).join(", ")}.`
    );
  }
